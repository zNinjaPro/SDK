...
function toWitnessSignalsWithdraw(
  ...
  // Compute canonical nullifier = Poseidon(commitment, nullifierKey)
  ...
  // Debug alternative (legacy style) including leaf index
  if (process.env.ZK_TRACE_MERKLE === "1") {
    try {
      const leafIndexBuf = Buffer.alloc(32);
      leafIndexBuf.write(
        inputs.merkleProof.leafIndex.toString(16).padStart(64, "0"),
        "hex"
      );
  ...
  // Match exact circuit signal names from withdraw.circom
...
function toWitnessSignalsTransfer(inputs: TransferInputs): Record<string, any> {
  ...
  // Match exact circuit signal names from transfer.circom
  return {
    // Public inputs
    merkleRoot: BigInt(
